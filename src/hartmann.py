...
# ✅ Load Emotion Model (ONLY using DistilRoBERTa)
...
def get_emotions(text, min_confidence=0.3):  # Reduced threshold to 0.1
    """Extract emotions with intensity and confidence scores using DistilRoBERTa."""
    ...
    # Ensure primary and secondary emotions are selected, avoiding "neutral" if possible
    primary_emotion = sorted_emotions[0] if sorted_emotions else ("neutral", 1.0)
    secondary_emotion = None

    for emotion in sorted_emotions[1:]:  # Look for the next best emotion
        if emotion[0] != "neutral":  
            secondary_emotion = emotion
            break

    # If no non-neutral secondary emotion is found, take the second one (even if it's neutral)
    if not secondary_emotion and len(sorted_emotions) > 1:
        secondary_emotion = sorted_emotions[1]  # Pick second-highest emotion
...
# ✅ Save to JSON
with open("adorescore_analysis.json", "w") as f:
